"""A tiny, self-contained MCP server — the demo connector for otto-agent.
...
so the connector story runs with zero extra installs:
    ...
    pip install -e '.[mcp]'
    cp examples/mcp.demo.json .otto/mcp.json
...
that's the whole point: connectors plug in without changing Otto's code.
"""
...
if __name__ == "__main__":
    mcp.run()  # stdio transport — how Otto's MCPBridge talks to it
