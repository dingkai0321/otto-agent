...
| `pi --mode json` | **event stream** on stdout, one JSON per line ← *otto plugs in here* |
...
```bash
claude -p "Reply with exactly: otto otto"   # answers, dies
claude    "Reply with exactly: otto otto"   # banner, state, waits for you
```
...
| sub-agents | spawn more pi's (tmux — or otto's `delegate_task`) |
...
## 4 · otto × pi — the collab thesis

```
otto loop → delegate_task → subprocess → pi -p --mode json
     pi's events → the arena card (live mini-terminal)
     pi's tokens → otto's usage ledger (coding runs aren't free)
```

Wiring: [`otto/tools/experimental.py`](../otto/tools/experimental.py) — every
...
| 4 | The League | **otto × pi** | `delegate_task` |
...
# 2b — guard a verb (throwaway dir with a dummy .otto)
pi -e .pi/extensions/pokemon-battle.ts … -p "Run: rm -rf .otto"
#  → "…blocked by a safety guard. The .otto directory is protected…"
#  → .otto SURVIVED. The refusal came back as a tool result and the model
...
### Stage 4 — otto × pi
```bash
OTTO_EXPERIMENTAL=1 make run        # + make dashboard (:7777)
# ask otto: "delegate to pi: what beats Charizard? use the pokedex + type_matchup"
```
...
- **pi runs the model raw, and it can wander.** On one run, haiku ignored "what
  ...
  Why: pi loads `AGENTS.md`/`CLAUDE.md` from the repo, and otto's says *"commit
