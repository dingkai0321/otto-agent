...
- [x] `otto dashboard` running on your own machine → `localhost:7777` in a real browser
...
| 4 | Calendar read | *"What's on my calendar today?"* | `list_events` fires; answers from PostgreSQL | [x] |
...
| 9 | **Voice** | `otto voice` — hands-free, say "otto otto, …" (or click the dock mic) | WAV → local Whisper; landed in a distinct `voice` conversation | [x] |
| 10 | Eval / LLM-Ops (hero 2) | `make gate` in a terminal | prints `GATE OPEN`; Ops ▸ Eval history gains a row | [x] |
| 11 | Spend ledger | (just look) | Ops: all-time cost/tokens, per-day — survives resets | [x] |
| 12 | **Database tab** | click each table tab; run a query in the **SQL console** | per-table schema (indigo headers) + rows; `SELECT` returns live data (and persists) | [x] |
| 13 | Ops walkthrough | (just look) | Ops: eval-history table, per-turn gate decisions, slowest turns, inline JSONL traces | [x] |
| 14 | Markdown chat + Gateway inbox | (bonus) | replies render bold/tables/lists; Gateway = channel-tagged conversation inbox, live | [x] |

## Status: all 14 beats dry-run verified ✅

Everything above works on the live dashboard. Two real gateways proven (web / voice), all four
pillars, both hero moments. Remaining is optional polish, not testing —
...
history for a clean take, and backs up the whole `.otto` first. The `usage.jsonl` spend ledger is
**kept** (it's a permanent record) unless you pass `--reset-spend`. It never deletes the db file, so a
running `otto dashboard`/`otto voice` keeps working. Nothing else clears your data.
