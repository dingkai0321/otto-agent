...
# The pi × otto video — shooting rundown
...
```
pi --mode json -p --no-session -nt "Reply with exactly: otto otto"
```
...
```
claude -p "Reply with exactly: otto otto"     # CLI: answers and dies
claude "Reply with exactly: otto otto"        # TUI: banner, state, waits
```
...
carries the model; pi shows you the model. That's why otto's arena delegates
coding to pi."*

## Segment D — otto hires pi (the demo)

```
OTTO_EXPERIMENTAL=1 make run
```
```
...
Receipts: `python -m otto.ops.show_trace` for the delegation in the trace.
