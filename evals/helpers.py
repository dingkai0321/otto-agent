...
real-Otto factory for live ones."""
...
def _has_key() -> bool:
    """True when the ACTIVE provider (OTTO_PROVIDER) has its key set, so live
...
def make_otto(home: Path, client=None, **settings_overrides):
    """Build a Otto with an isolated home dir; optionally swap in a fake client."""
    from otto.app import Otto
    from otto.config import Settings

    # A test must describe its own world. `otto/config.py` calls load_dotenv()
    ...
    #                          a stale OTTO_GRAPH_WORKFLOWS=1 ate a scripted
    ...
    return Otto(settings=settings, client=client)
