...
otto runs on people's own machines with their own keys, and it writes a lot of
...
The gap this was written for (2026-07-26): .gitignore listed `.otto/` by exact
...
second home was called `.otto-discord/`, matched nothing, and its SOUL.md,
usage.jsonl and outbox/ were tracked. A privacy fix had quietly created a
different privacy hole. `.otto-*/` closes it for any alternate home.
...
# alternate one (DISCORD_HOME, or any OTTO_HOME a user points elsewhere).
ARTIFACTS = ["MEMORY.md", "SOUL.md", "usage.jsonl", "calendar.ics",
             "traces/2026-07-26.jsonl", "outbox/msg-1.txt"]


@pytest.mark.parametrize("home", [".otto", ".otto-discord", ".otto-demo"])
...
def test_the_env_template_carries_no_real_values():
    ...
    # OTTO_PROVIDER is a non-secret default and is meant to have a value.
    unexpected = [ln for ln in filled if not ln.startswith("OTTO_PROVIDER=")]
