...
from `load_settings()`, which reads OTTO_EXPERIMENTAL from the environment.
...
def test_an_explicit_setting_beats_the_global_env_switch(tmp_path, monkeypatch):
    ...
    OTTO_EXPERIMENTAL, then switching chat delegation on (which writes that var)
    ...
    monkeypatch.setenv("OTTO_EXPERIMENTAL", "1")      # global switch ON
...
def test_env_var_is_what_load_settings_reads(monkeypatch):
    """Why the chat was broken: nothing sets OTTO_EXPERIMENTAL for the server."""
    monkeypatch.delenv("OTTO_EXPERIMENTAL", raising=False)
    assert load_settings().experimental is False
    monkeypatch.setenv("OTTO_EXPERIMENTAL", "1")
    assert load_settings().experimental is True
    assert os.getenv("OTTO_EXPERIMENTAL") == "1"
