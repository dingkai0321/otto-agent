...
1. THE WHEEL SHIPPED NO SKILLS. `pyproject.toml` packages `otto`, but the
   bundled skills live in `skills/` at the REPO ROOT, so `pip install
   otto-agent` produced a Otto with zero procedural memory — one of the four
   pillars, silently absent. The dashboard shipped fine (it lives under
   `otto/ops/static`), which is exactly why nobody noticed: the thing you can
...
2. THE TEST SUITE READ THE DEVELOPER'S `.env`. `otto/config.py` calls
   load_dotenv() at import, so a stale `OTTO_GRAPH_WORKFLOWS=1` routed every
...
def test_the_bundled_skills_are_findable():
    """Whatever the install shape, Otto must find the skills it ships with."""
    dirs = bundled_skill_dirs()
    assert dirs, (
        "bundled_skill_dirs() found nothing — Otto would start with no "
...
def test_the_wheel_carries_the_skills_folder():
    """The build config half of the fix. `packages = ["otto"]` alone leaves the
    repo-root skills/ out of the wheel, which is the bug — force-include copies
    it to otto/skills so an installed Otto finds it beside the code."""
    wheel = _pyproject()["tool"]["hatch"]["build"]["targets"]["wheel"]
    assert wheel.get("force-include", {}).get("skills") == "otto/skills", (
        "pyproject no longer force-includes skills/ into the wheel — a "
        "`pip install otto-agent` would ship with zero skills again"
    )


def test_lookup_covers_both_install_shapes():
    """The lookup half. A checkout has skills/ at the repo root; a wheel has it
    at otto/skills. Exactly one exists at a time, so the function must look in
    both — checking only one is how this broke."""
    import inspect

    from otto import memory
...
def test_evals_never_inherit_the_developers_env():
    """THE regression for #2. `make_otto` must pin every switch that changes
    ...
    src = inspect.getsource(helpers.make_otto)
    for switch in ("apple_calendar", "google_calendar", "apple_tools", "graph_workflows"):
        assert switch in src, (
            f"make_otto no longer pins {switch!r} — a stale value in the "
...
def test_a_scripted_turn_ignores_the_graph_flag(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OTTO_GRAPH_WORKFLOWS", "1")
