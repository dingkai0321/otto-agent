"""DETERMINISTIC EVAL — the delegated-coding workspace (otto.tools.workspace).
...
from otto.tools import workspace as ws


def test_run_folder_is_dated_and_named(tmp_path, monkeypatch):
    monkeypatch.setenv("OTTO_WORKSPACE", str(tmp_path))
...
def test_autorun_disabled_by_env(tmp_path, monkeypatch):
    monkeypatch.setenv("OTTO_WORKSPACE", str(tmp_path))
    monkeypatch.setenv("OTTO_DELEGATE_AUTORUN", "0")
