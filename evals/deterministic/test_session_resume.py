...
from otto.ops.browser_agent import resume_or_new_session


def _seed(app, session_id, age_minutes, source="dashboard"):
    app.conn.execute(
        "INSERT INTO chat_log (role, content, session_id, created_at, source) "
        "VALUES ('user', 'hi', %s, now() - (%s * interval '1 minute'), %s)",
        (session_id, age_minutes, source),
    )
    app.conn.commit()
...
def test_only_dashboard_source_threads_are_resumed(tmp_path, monkeypatch):
    """A recent voice/cli thread must not hijack the dashboard's resume —
    matched by source, not id."""
    monkeypatch.setenv("OTTO_SESSION_IDLE_MINUTES", "60")
    app = make_otto(tmp_path / "home", client=ScriptedClient([]))
    _seed(app, "voice-12345", age_minutes=1, source="voice")
    got = resume_or_new_session(app.conn)
    assert got.startswith("dashboard-") and got != "voice-12345"
