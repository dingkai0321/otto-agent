...
it keeps a single Otto behind `otto.ops.browser_agent`. Two callers mutate it:
...
fresh Otto starts on the eternal 'default' session. So one visit to the Settings
...
def test_a_failed_rebuild_keeps_the_working_agent(isolated, monkeypatch):
    ...
    monkeypatch.setattr("otto.app.Otto", explode)
