...
from evals.helpers import HAS_KEY, ScriptedClient, make_otto, response, text_block, tool_block
...
def test_create_event_writes_db_and_ics(tmp_path):
    ...
    app = make_otto(tmp_path / "home", client=ScriptedClient([gate] + turn))
...
def test_iteration_guardrail_stops_runaway_loop(tmp_path):
    ...
    app = make_otto(tmp_path / "home", client=ScriptedClient([gate] + runaway), max_iterations=3)
