"""Skill installer — try anyone's skill in one command.
    ...
    python -m otto skill install https://github.com/<user>/<repo>/blob/main/skills/foo/SKILL.md
    python -m otto skill install https://gist.github.com/<user>/<id>

Downloads the SKILL.md, validates the frontmatter (same check CI runs on
community PRs), and drops it in OTTO_HOME/skills/<name>/ where the loader
...
from otto.memory.procedural.loader import _parse
...
def install(url: str) -> None:
    ...
    print("It loads next time Otto starts. Read it first — skills are instructions.")
