...
Pick with OTTO_PROVIDER=anthropic|openai|gemini|deepseek|minimax|kimi|glm|openrouter
and set that provider's API key in .env. Override the model ids with OTTO_MODEL /
OTTO_SMALL_MODEL if the defaults below age out — they're just strings. This
matters most for openrouter: it's a single key in front of hundreds of models,
so OTTO_MODEL=<vendor>/<model> (e.g. "google/gemini-3.5-flash") picks whichever
...
PROVIDERS: dict[str, Provider] = {
    ...
    # /v1/chat/completions (they need /v1/responses), so every Otto turn 400s on
    ...
    # checked). Override with OTTO_SMALL_MODEL=kimi-k3 if your key is K3-only.
...
def _no_key_message(name: str, key_env: str) -> str:
    ...
    the fact that Otto speaks to eleven providers, not one.
    """
    from otto.config import DOTENV_PATH
    ...
    return (
        ...
        f"Switch with OTTO_PROVIDER=<name> and that provider's key."
    )
...
def get_client(settings: Settings):
    ...
    if provider is None:
        raise SystemExit(f"Unknown OTTO_PROVIDER '{settings.provider}'. "
    ...
    timeout = float(os.getenv("OTTO_LLM_TIMEOUT", "120"))
