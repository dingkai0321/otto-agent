"""Generate the current Otto architecture whiteboard.

Run:
    python -m otto.ops.whiteboard.build_otto_architecture
...
BOARD = ROOT / "docs" / "whiteboards" / "otto-architecture.excalidraw"
SVG_PROXY = Path("/tmp/otto-architecture.svg")
...
def build_elements() -> list[dict]:
    ...
    # Title and identity.
    a(S.text(60, 36, "Otto — one turn through the whole system", size=S.FS_TITLE))
    ...
    a(S.text(2160, 52, "GitHub  @dingkai0321", size=S.FS_BODY))

    # ------------------------------------------------------------------ Harness
    a(S.boundary(40, 190, 2440, 470, "HARNESS — otto/app.py", color="red"))

    a(S.pill_header(80, 250, 330, "1  GATEWAYS", color="grey"))
    a(S.labeled_box(80, 320, 330, 210,
                    "text in / text out\n\nCLI · Dashboard · Voice\nDiscord · WhatsApp\n\notto/gateway/ + ops/dashboard.py",
    ...
    a(S.labeled_box(800, 320, 350, 210,
                    "runtime/session.py\n\nSOUL.md + Skill catalog\nlast N chat turns\ngated long-term memory\ntrailing Agent Status Bar",
    ...
    a(S.labeled_box(1690, 295, 330, 230,
                    "ToolRegistry\n\ngeneral: file · terminal · python · fetch\ndomain: memory · tasks · calendar\nprogressive: load_skill + resources\nsubagents · Apple · MCP",
    ...
    a(S.annotate(1280, 535, "hooks → permission → execute · stop: no tool call OR guardrail"))
    ...
    # ------------------------------------------------------------------- Memory
    a(S.boundary(40, 710, 1490, 580, "MEMORY — otto/memory/", color="green"))

    a(S.labeled_box(80, 800, 330, 150,
                    "PROCEDURAL\nSKILL.md packages\nmetadata always · body on demand",
                    color="green", size=S.FS_BODY))
    a(S.labeled_arrow(245, 800, 930, 530, "catalog into prompt",
                      color=S.PAL["green"][1]))
    ...
    a(S.labeled_box(1170, 755, 300, 180,
                    "PostgreSQL + pgvector\n\nchat · facts · episodes · calendar\nknowledge documents + chunks\nGIN full-text · HNSW vectors\n\nMEMORY.md = readable mirror",
                    color="grey", size=S.FS_BODY))
    ...
    # -------------------------------------------------------------- Eval / Ops
    a(S.boundary(1590, 710, 890, 580, "EVAL / LLM OPS — otto/ops/ + evals/", color="blue"))
    ...
    a(S.source_label(60, 1330,
                     "source: otto/app.py, runtime/*, loop/agent.py, memory/*, tasks/*, tools/*, ops/* — 2026-08-06"))
    a(S.text(2280, 1330, "@dingkai0321", size=S.FS_BODY, color=S.PAL["grey"][1]))
    return e
