...
aligned rows each, then the otto punchline. Facts are sourced on the board
...
Run:  python -m otto.ops.whiteboard.build_pi_vs_claude
"""
...
ROWS = [
    ...
    ("SUB-AGENTS",
     ...
     "refused — spawn pi yourself: tmux,\nor otto's delegate_task (that's us)",
     "plain"),
    ("MCP",
...
def build() -> list:
    ...
    e += S.labeled_box(80, band, 2410, 96,
                       "otto uses BOTH: Claude Code writes otto's code - pi is otto's delegate_task sub-agent,\n"
