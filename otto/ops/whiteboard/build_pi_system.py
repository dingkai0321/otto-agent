...
One tight landscape teaching board (companion to docs/whiteboards/pi-chart-prompt.md):
  ...
  FOOTERS     refused-on-purpose (red)  ·  otto x pi (blue)
...
Run:  python -m otto.ops.whiteboard.build_pi_system
"""
...
def build():
    ...
    # 3 container boundaries (loop / refused / otto), the pink model, the green
    ...
    a(S.text(78, 408, "↑ otto plugs in here", size=S.FS_SMALL, color=PAL["green"][1]))
    ...
    a(S.boundary(660, 925, 900, 205, "otto × pi", color="blue"))
    a(S.text(688, 985,
             "otto → delegate_task → pi -p --mode json\n"
             "events → arena card\n"
             "tokens → usage ledger",
             size=S.FS_BODY, color=INK))
