"""Generate the two K3 tutorial whiteboards in Sean's style.
  ...
  Act 2  — the GRADING sheet: test each claim live on Otto (10 min)
...
Run:  python -m otto.ops.whiteboard.build_k3_tutorial
"""
...
def act2(standalone=True):
    ...
    a(S.text(60, 40, "Testing the Hero -- live, on Otto", size=S.FS_TITLE))
    a(S.underline(64, 120, 760, color=S.PAL["orange"][1]))
    if standalone:
        a(S.socials_block(1640, 44))

    # B1 the rig: prompt chip -> Otto ellipse -> three provider chips
    a(S.text(80, 170, "B1   The rig = Otto", size=S.FS_HEADER, color=S.PAL["blue"][1]))
    a(S.chip(80, 250, 200, 90, "one prompt", color="green"))
    a(S.ellipse(360, 235, 220, 120, "Otto\n(swap the brain)", color="pink"))
