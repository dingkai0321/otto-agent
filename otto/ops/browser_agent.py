"""The browser gateway's agent — one Otto, shared by every tab.

The dashboard is a gateway like the CLI or Discord: it only moves text. But
...
across page refreshes, so it needs two things the other gateways don't:
  ...
  turns run one at a time. Correct for a single-user local tool and for sharing
  one PostgreSQL transaction connection across dashboard worker threads.
...
def resume_or_new_session(conn) -> str:
    ...
    idle_min = int(os.getenv("OTTO_SESSION_IDLE_MINUTES", "60"))
...
def get_agent():
    ...
    if _agent is None:
        ...
        _agent = Otto(settings=settings, conn=conn)
...
def maybe_rotate_session(agent) -> None:
    ...
    current session's newest message is older than OTTO_SESSION_IDLE_MINUTES
    ...
    if idle_min <= 0:
        return
    row = agent.conn.execute("SELECT MAX(created_at) AS last_at FROM chat_log WHERE session_id=%s",
...
def rebuild() -> str | None:
    ...
    with agent_lock:
        ...
        try:
            ...
            settings.ensure_home()
            conn = connect(settings.home, check_same_thread=False)
            fresh = Otto(settings=settings, conn=conn)
            # Carry the CONVERSATION across the swap. A settings change swaps the
            # brain, not the thread — but a brand-new Otto starts on the eternal
