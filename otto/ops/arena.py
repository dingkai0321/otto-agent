...
demo: `.otto/` is never opened here.
...
Two scores, deliberately separate:
  Completion  deterministic — did the right tool fire, with the right args?
              (otto.ops.scoring, only for prompts in the battery)
  Quality     an LLM referee's grade (otto.ops.judge), run AFTER the race as one
              gentle pass so a burst of concurrent calls can't 429 half of them.

Results land in the arena's own JSONL scoreboard (otto.ops.compare_history) —
never the PostgreSQL memory tables. dashboard.py owns the HTTP/SSE plumbing; this module owns the
race.
"""
...
from otto.ops import compare_history, scoring
from otto.ops import judge as judge_mod
from otto.ops.pricing import cutoff_for, price_for
...
def compare_stream(message: str, specs: list, emit, judge: bool = False,
    ...
    def run(spec):
        ...
        try:
            ...
            # EACH model writes its own event to the real 'Otto' calendar.
            settings = Settings(
                provider=provider,
                model=model,
                small_model="",
