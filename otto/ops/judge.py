...
Completion (otto.ops.scoring) is deterministic — did the right tool fire. Quality
...
pure text, no tools). Switchable per-race from the arena, or via OTTO_JUDGE_*.
Any provider works — Otto's OpenAI-compat client exposes the same
...
JUDGE_PROVIDER = os.getenv("OTTO_JUDGE_PROVIDER", "openai")
JUDGE_MODEL = os.getenv("OTTO_JUDGE_MODEL", "gpt-5.6-sol")
...
_JUDGE_SEM = threading.Semaphore(int(os.getenv("OTTO_JUDGE_CONCURRENCY", "2")))
