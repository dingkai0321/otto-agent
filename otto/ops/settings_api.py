...
from otto.ops import browser_agent, catalog
...
def settings_info() -> dict:
    ...
    return {
        ...
        "pinned": pinned,
        # a custom endpoint (e.g. OpenRouter) set via OTTO_BASE_URL / OTTO_API_KEY
        ...
        # episodic-memory backend: PostgreSQL (default) or Notion
...
def apply_settings(payload: dict) -> dict:
    ...
    if episodic_store is not None and episodic_store not in ("postgres", "notion"):
        return {"error": f"unknown episodic_store {episodic_store}"}
    before = {"provider": os.getenv("OTTO_PROVIDER", ""),
              "model": os.getenv("OTTO_MODEL", ""),
              "small_model": os.getenv("OTTO_SMALL_MODEL", "")}
    writable = ({"OTTO_PROVIDER", "OTTO_MODEL", "OTTO_SMALL_MODEL", "TAVILY_API_KEY",
                 "OTTO_EPISODIC_STORE", "OTTO_EXPERIMENTAL", "OTTO_GRAPH_WORKFLOWS",
                 "NOTION_TOKEN", "NOTION_EPISODES_DATABASE_ID"}
                | {p.key_env for p in PROVIDERS.values()})
    env_path = find_dotenv(usecwd=True) or ".env"

    updates = {"OTTO_PROVIDER": provider,
               "OTTO_MODEL": payload.get("model", "") or "",
               "OTTO_SMALL_MODEL": payload.get("small_model", "") or ""}
    if episodic_store:
        updates["OTTO_EPISODIC_STORE"] = episodic_store
    ...
    if experimental is not None:
        updates["OTTO_EXPERIMENTAL"] = "1" if str(experimental).strip() else ""
    graph_workflows = payload.get("graph_workflows")  # same is-not-None rule
    if graph_workflows is not None:
        updates["OTTO_GRAPH_WORKFLOWS"] = "1" if str(graph_workflows).strip() else ""
    ...
    if provider != before["provider"]:
        if updates["OTTO_MODEL"] in ("", before["model"]):
            updates["OTTO_MODEL"] = catalog.default_model_for(provider)
        if updates["OTTO_SMALL_MODEL"] in ("", before["small_model"]):
            updates["OTTO_SMALL_MODEL"] = ""
    for k, v in (payload.get("keys") or {}).items():
        if k in writable and v:  # only non-empty keys overwrite
            if k == "NOTION_EPISODES_DATABASE_ID":
                from otto.memory.episodic.notion_store import normalize_database_id
    ...
    browser_agent.current().emit(
        HookEvent.CONFIG_CHANGE,
        **{
            "from": before,
            "to": {"provider": provider, "model": updates["OTTO_MODEL"],
                   "small_model": updates["OTTO_SMALL_MODEL"]},
        },
    )
    return {"ok": True, **settings_info()}
