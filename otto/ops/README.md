# otto/ops — the Eval/LLM-Ops pillar
...
whole directory and otto still runs. That's deliberate: ops observes, it never
participates.
...
| `browser_agent.py` | The ONE shared `Otto` behind the browser gateway + its dated chat session. |
...
| `compare_history.py` | The arena's own JSONL scoreboard. Never PostgreSQL. |
...
| `show_trace.py` | `otto trace` — reading those files back in the terminal. |
...
```python
from otto.ops import browser_agent
browser_agent.current()          # sees a later swap
```

```python
from otto.ops.browser_agent import _agent   # frozen at None forever
```
