"""`python -m otto brief` — a morning briefing that runs through the normal
...
def main() -> None:
    console = Console()
    otto = Otto()
    if not otto.settings.apple_tools:
        console.print("[dim]Tip: set OTTO_APPLE_TOOLS=1 to brief from your real Calendar and Mail.[/dim]")
    result = otto.respond(PROMPT, source="brief")
    console.print(result.reply)
    out = otto.settings.home / "outbox" / f"brief-{date.today().isoformat()}.txt"
