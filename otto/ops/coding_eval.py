...
The agentic battery (evals/dataset.jsonl) scores whether a model drives Otto's
...
Otto stays the orchestrator; pi stays the contractor — we just get to compare
...
# Otto provider id -> pi's built-in provider id (see `pi --list-models`).
PI_PROVIDER = {
