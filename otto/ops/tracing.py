...
1. JSONL, always on: every turn appends readable lines to
   .otto/traces/<date>.jsonl. A trace is just "what happened, in order" —
...
2. OpenTelemetry spans, when OTEL_EXPORTER_OTLP_ENDPOINT is set: the same
   events as a span tree any OTel backend can render. For a local dashboard:
       ...
       pip install 'otto-agent[tracing]'
       phoenix serve                                # localhost:6006
       OTEL_EXPORTER_OTLP_ENDPOINT=http://localhost:4317 python -m otto
...
class TraceEncodingError(UnicodeError):
    ...
    def __init__(self, path: Path):
        ...
        super().__init__(
            ...
            "Otto version using the Windows system encoding. Move it out of the traces directory "
            "and keep it as a backup, then restart Otto if it is today's trace. The file "
            "was not modified."
        )
...
class Tracer:
    """Read-only hook adapter: every lifecycle event lands in the trace."""
    ...
    # ---- read-only hook adapter (legacy observer signature at the boundary)
        ...
        if self._otel_tracer and self._span_ctx is not None:
            with self._otel_tracer.start_as_current_span(
                ...
                attributes={
                    ...
                    **{f"otto.{k}": json.dumps(v, default=str) for k, v in event.items()},
                },
            ):
                pass
    ...
    def turn(self, user_message: str):
        ...
        if self._otel_tracer:
            with self._otel_tracer.start_as_current_span(
                "agent_run",
                attributes={"openinference.span.kind": "AGENT", "otto.user_message": user_message},
            ) as span:
                self._span_ctx = span
                try:
                    yield self
