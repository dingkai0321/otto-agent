...
2. **What you chose.** `.otto/models.json` holds an ordered `provider:model`
...
from otto.ops.pricing import remember_price
...
def list_models(provider: str | None = None) -> dict:
    ...
    OpenAI-compatible endpoints (OpenRouter, Gemini, any OTTO_BASE_URL), or the
    ...
    # OTTO_BASE_URL only applies to the provider it was set for).
    ...
    req = urllib.request.Request(url, headers={
        ...
        "User-Agent": "Mozilla/5.0 (compatible; Otto)",
    })
    try:
...
def pinned_specs() -> list[str]:
    ...
    .otto/models.json. The chat switcher shows exactly these. Before they've
