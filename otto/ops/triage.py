...
from otto.hooks import build_hooks
from otto.ops.browser_agent import agent_lock, get_agent
...
def run_triage(observer=None, message: str = "") -> dict:
    ...
    def capture(kind: str, ev: dict) -> None:
        if kind == "route":
    ...
    with agent_lock:
        agent = get_agent()
        hooks = build_hooks(agent.tools.permission_policy, observer, capture)
        ...
        result = agent._respond_via_graph(text, hooks, stream=False)
