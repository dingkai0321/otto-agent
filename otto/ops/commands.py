...
Two kinds of graph ship in Otto and conflating them made the UI say something
...
DISCOVERY, and why it is shaped this way: a module in otto/graph/workflows/ is
...
makes it testable. What makes a workflow runnable is its BINDER in otto/ops/,
where real callables meet the pure shape.

So the rule is: `otto/graph/workflows/<name>.py` plus `otto/ops/<name>.py`
...
def describe() -> str:
    ...
    lines = [
        "**Built-in commands**",
        "",
        "`/context` — show the current context estimate and compaction threshold",
        "`/compact [instructions]` — summarize active working history now",
        "",
        "**Graph workflows you can run by name**",
        "",
    ]
...
def unknown_reply(name: str) -> str:
    known = sorted({*discover(), "compact", "context"})
