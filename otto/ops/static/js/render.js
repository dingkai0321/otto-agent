// otto dashboard — formatters + chat card renderers + chatlog + streaming + send.
...
function histItem(m){
  ...
  if (m.meta) return {role:"otto", reply:m.content, gate:m.meta.gate,
                      graph:m.meta.graph,
                      tools:m.meta.tools, iterations:m.meta.iterations,
                      latency_ms:m.meta.latency_ms, model:m.meta.model};
  return {role:"otto", reply:m.content, historical:true};
}
...
const streamingCard = m => `<div class="card">
  ...
  ${m.stream
     ...
     : `<div class="meta" style="margin:0">thinking&hellip;${m.started?` ${Math.round((Date.now()-m.started)/1000)}s`:""}${
         ...
         ? `<br>still waiting: slow models (free tiers especially) can queue for a while; this errors out at the OTTO_LLM_TIMEOUT limit instead of hanging forever`
         : ""}</div>`}
</div>`;
...
const liveTaskRows = tasks => (tasks||[]).map(t => `<div class="tool ok">
  <div class="tool-head"><span class="dot ${t.status==="completed"?"ok":""}"></span>
    <code>#${esc(t.id)}</code> ${esc(t.subject||"")} <span class="meta">${esc(t.status||"")}</span></div>
</div>`).join("");

function renderChatLog(){
  if (!CHAT.length)
    return `<div class="empty" style="padding:6px 2px">Message Otto here from any tab. Open Overview to watch it flow through the harness, or the Gateway tab to see every channel's messages together.</div>`;
...
function applyStreamEvent(pending, ev){
  ...
  if (ev.kind === "gate") pending.gate = {decision: ev.decision, reason: ev.reason};
    ...
    pending.permission = {tool: ev.tool, reason: ev.reason, decision: "ask"};
    const detail = JSON.stringify(ev.args || {}, null, 2);
    const allow = window.confirm(
      `Otto permission required\n\n${ev.reason}\n\n${ev.tool}(${detail})\n\nAllow this tool call?`
    );
    pending.permission.decision = allow ? "allow" : "deny";
    fetch("/api/permission", {method:"POST", headers:{"Content-Type":"application/json"},
      body:JSON.stringify({request_id:ev.request_id, decision:allow?"allow":"deny"})});
  }
    ...
    const task = ev.task || {};
    const tasks = pending.tasks = pending.tasks || [];
...
async function sendChat(fromInput){
  ...
  const pending = {role:"otto", pending:true, stream:"", started: Date.now()};
  CHAT.push(pending);
  syncChatLogs();
