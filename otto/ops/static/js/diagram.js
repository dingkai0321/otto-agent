// otto dashboard — the architecture SVG (archSVG, byte-frozen) + its live animation.
...
function archSVG(d){
  ...
  return `<div style="overflow-x:auto"><svg viewBox="0 -10 1044 674" class="arch" role="img">
    ...
    ${lbl(384,48,"LOOP")}
    ${box(384,64,140,44,"LLM agent","reason","loop","","llm")}
    ${box(392,118,124,44,"Hooks","pre · post · stop","loop","","hooks")}
    ${box(384,172,140,44,"Tools","create_event…","tools","","tools")}
    ${flow("M448 108 L448 118")}${flow("M448 162 L448 172")}
    ${flow("M470 172 L470 162")}${flow("M470 118 L470 108")}
    ...
    <!-- The gateway is the door IN and OUT: the reply leaves through the very
         gateway it arrived at (Discord sends it, the CLI prints it, voice
    ...
    ${box(264,452,204,72,"Semantic · PostgreSQL","durable facts · "+d.facts.length+" facts","memory/semantic","","semantic")}
...
const STAGE = {
  ...
  permission:    {nodes:["hooks"],                   edges:[],                          label:"permission hook"},
  tool:          {nodes:["hooks","tools"],           edges:[],                          label:"hooks → tool"},
