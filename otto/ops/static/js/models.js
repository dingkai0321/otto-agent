// otto dashboard — settings write (applyModel), model picker/catalog/pins.
...
// Otto's loop needs tool calling, so tool-less models are labelled as such.
...
async function loadModelList(){
  ...
  if (modelCatalog.listed){
    ...
    msg.textContent = `${ms.length} models on ${modelCatalog.endpoint}` +
      (free.length ? ` · ${free.length} free, ${freeTools.length} of those tool-capable (Otto needs tool calling)` : "") +
