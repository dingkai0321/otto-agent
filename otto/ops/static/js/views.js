// otto dashboard — subtab/db helpers, SQL console, Memory/Tools sub-views, VIEWS.
...
// A raw PostgreSQL table, scrollable, with the column names AS the sticky
...
const DB_DESC = {
  ...
  knowledge_documents: "imported PDF, DOCX, Markdown, and text sources",
  knowledge_chunks: "citation-bearing chunks with full-text and pgvector indexes",
  task_lists: "session/shared scopes for durable task lists",
  tasks: "durable task records, status, owner, and acceptance details",
  task_dependencies: "DAG edges: each task points at the tasks blocking it",
};
...
function dbQueryView(){
  return `<div class="meta" style="margin-bottom:10px">A read-only SQL console over the active PostgreSQL schema.
      Only <code>SELECT</code> runs inside a read-only transaction,
...
// the Data tab shows the SAME rows as raw PostgreSQL tables (see the explainer).
function memOverview(d){
  ...
  const pillars = [
    ...
    ["Knowledge","knowledge",(d.knowledge||[]).length+" documents","PDF/DOCX/text chunks — PostgreSQL full-text + pgvector"],
  ...
  return `<div class="card" style="border-color:var(--accent);background:var(--accent-soft)">
      <b>Memory vs Database — two views of one PostgreSQL schema.</b>
      <div class="r">This tab is the curated, per-pillar view of what Otto remembers. The
      <a class="reveal" onclick="location.hash='database'">Database tab</a> shows the exact same
      thing as raw PostgreSQL tables (plus full-text and HNSW indexes) — different altitude.
      <br><br>Some assistants (Hermes) keep memory as a single <code>MEMORY.md</code> file. Otto keeps
      the queryable source in PostgreSQL (facts + episodes, full-text searchable) <b>and</b> writes a
    ...
    <div class="meta" style="margin-top:14px">Files: ${reveal("MEMORY.md","MEMORY.md")} · ${reveal("SOUL.md","SOUL.md")} · ${reveal("skills","skills/")} · structured state: PostgreSQL</div>`;
}
function memSemantic(d){
  let h = `<div class="meta" style="margin-bottom:12px">Durable facts distilled from what you tell Otto —
...
function memEpisodic(d){
  const src = d.episodes_source || "postgres";
...
function memSkills(d){
  let h = `<div class="meta" style="margin-bottom:12px">Procedural memory — every skill's name and
    description form a compact catalog; <code>load_skill</code> activates the core instructions,
    then referenced files load individually through <code>load_skill_resource</code>. Specialist
    tool schemas can appear only after activation. Add your own three ways: teach Otto in chat (it calls
...
function memKnowledge(d){
  const docs = d.knowledge || [];
  let h = `<div class="card" style="border-color:var(--accent);background:var(--accent-soft)">
    <b>Knowledge RAG is separate from personal memory.</b><div class="r">Documents are parsed into
    citation-bearing chunks and searched with reciprocal-rank fusion over PostgreSQL full-text and
    pgvector cosine similarity. Import with <code>otto knowledge add handbook.pdf policy.docx</code>.
    Set <code>OPENAI_API_KEY</code> or <code>OTTO_EMBEDDING_API_KEY</code> for vector embeddings.</div></div>`;
  h += `<div class="card" style="padding:4px 8px"><table><tr><th>document</th><th>chunks</th><th>source</th></tr>${
    docs.map(doc => `<tr><td><code>${esc(doc.title)}</code></td><td class="meta">${doc.chunk_count}</td>
      <td class="meta">${esc(doc.source)}</td></tr>`).join("")}</table></div>`;
  return h || `<div class="card empty">no documents imported</div>`;
}
function memSoul(d){
  return `<div class="meta" style="margin-bottom:12px">SOUL.md is Otto's persona — the system prompt it
    loads every turn. Editing it changes who your Otto is. Changes are live next turn.</div>
...
function toolsMCP(t){
  ...
  let h = `<div class="card ${m.configured?"":""}" style="border-color:${m.live?"var(--good)":"var(--line2)"}">
    ...
    <div class="r">MCP lets Otto borrow tools from any external server (files, GitHub, a database, …),
  ...
  h += `<h2>Connect one (30 seconds)</h2><div class="card">
    ...
    <div class="meta" style="margin-top:6px">2 — create ${reveal("","the .otto folder")}<code>/mcp.json</code>:</div>
  ...
  h += `<div class="meta" style="margin-top:12px">The same pattern scales: any MCP server (yours or a vendor's)
    plugs in the same way — no code changes to Otto. Skills work the same way — drop a <code>SKILL.md</code>
    in ${reveal("skills","skills/")}.</div>`;
  return h;
}

const VIEWS = {
  // Gateway: ONE unified conversation across every channel (dashboard, discord,
  // whatsapp, voice, cli) — the same loop + memory answer all of them. Each message is
  ...
  gateway(d){
    ...
    let h = `<div class="meta" style="margin-bottom:14px">Every conversation across every channel —
      web, Discord, WhatsApp, voice, terminal — answered by the same brain. Click one to open it in the
  ...
  overview(d){
    ...
    ${graphPanel(d)}
    <h2>Latest turn</h2>${d.turns.length?turnCard(d.turns[0]):'<div class="card empty">no turns yet — talk to Otto first</div>'}`;
  },
  loop(d){
  ...
  tasks(d){
    const tasks = d.tasks || [];
    let h = `<div class="card" style="border-color:var(--accent);background:var(--accent-soft)">
      <b>One store, two scales.</b><div class="r">A flat list gives one agent Todo-style progress.
      Add <code>blockedBy</code> edges and owners and the same records become a persistent task DAG.
      PostgreSQL transactions make claiming atomic; tasks survive restarts and context compaction.</div></div>`;
    if (!tasks.length) return h + `<div class="card empty">no tasks in this session — Otto creates them for complex multi-step work</div>`;
    h += `<div class="meta" style="margin:12px 0">session <code>${esc(d.current_session||"")}</code></div>`;
    h += tasks.map(t => `<div class="toolcard">
      <div class="tn"><code>#${t.id}</code> ${esc(t.subject)}
        <span class="srcpill ${t.status==="completed"?"":"apple"}">${esc(t.status)}</span></div>
      <div class="td">${esc(t.description||t.active_form||"")}</div>
      <div class="meta" style="margin-top:6px">owner: ${esc(t.owner||"unassigned")} · blockedBy:
        ${t.blockedBy&&t.blockedBy.length?t.blockedBy.map(x=>`#${x}`).join(", "):"none"} ·
        ${t.available?"available":t.blocked?"blocked":"not claimable"}</div></div>`).join("");
    return h;
  },
  ...
  graph(d){
    ...
    if (!g.enabled)
      h += `<div class="card"><b>Off</b> — every turn currently runs the classic loop.
        ...
        <code>OTTO_GRAPH_WORKFLOWS=1</code> in <code>.env</code>. Any failure anywhere fails open to the
  ...
  memory(d, sub){
    ...
    const tabs = [["overview","Overview"],["semantic","Semantic",d.facts.length],
      ["episodic","Episodic",d.episodes.length],["knowledge","Knowledge",(d.knowledge||[]).length],
    ...
    if (sub==="knowledge") return h + memKnowledge(d);
  ...
  settings(d){
    ...
    h += `<h2>Provider &amp; keys (BYOK)</h2><div class="card">
      ...
      ${st.base_url?`<div class="meta" style="margin:4px 0 8px">Custom endpoint active: <code>${esc(st.base_url)}</code> (OTTO_BASE_URL${st.custom_key_set?" + OTTO_API_KEY":""}). The model list below comes from it.</div>`:""}
    ...
    <h2>Episodic memory</h2><div class="card">
      <div class="meta" style="margin-bottom:8px">Where dated episode summaries live. Default is the local
        PostgreSQL. Pick <code>notion</code> to store them in a Notion database instead
        (requires <code>pip install -e '.[notion]'</code>).</div>
      <label class="fld">Backend
        <select id="set-episodic-store" onfocus="markEditing()">
          <option value="postgres" ${st.episodic_store!=="notion"?"selected":""}>postgres — local PostgreSQL (default)</option>
    ...
    <h2>Web search key (optional)</h2><div class="card">
      ...
      <div class="meta" style="margin-top:10px">Note: running terminal, voice, Discord, and WhatsApp gateways keep their old provider until restarted.</div>
    </div>`;
    return h;
  },
  tools(d, sub){
    ...
    h += `<div class="meta" style="margin-bottom:12px">The capabilities the agent can call this turn.
      ...
      ${t.apple_on?"":"Apple tools are off (set <code>OTTO_APPLE_TOOLS=1</code>). "}Connect more via
      <a class="reveal" onclick="location.hash='tools/mcp'">MCP</a>.</div>`;
    const SRC = [["general","General execution — Skills use these"],
      ["flagship","Flagship task — scheduling"],["tasks","Durable task system"],
      ["subagents","Subagents — isolated specialist loops"],["web","Web search"],
    ...
    if ((t.planned||[]).length){
      h += `<h2>Coming soon <span class="meta" style="font-weight:400">· architecture placeholders, not callable yet</span></h2>`;
  ...
  database(d, sub){
    // The persistence layer itself — PostgreSQL tables and indexes.
    ...
    if (sub !== "overview"){
      ...
      const notionNote = (t.name === "episodes" && d.episodes_source === "notion")
        ? `<div class="meta" style="margin-bottom:10px">Episodes currently live in Notion — see
            ...
            The rows below are the PostgreSQL copy.</div>` : "";
    ...
    h += `<div class="card" style="border-color:var(--accent);background:var(--accent-soft)">
      <b>Database vs Memory.</b> <span class="r">This is the raw persistence layer — the literal PostgreSQL
      ...
      uses a <code>MEMORY.md</code> file, Otto uses these queryable tables — and mirrors them to a
      readable <code>MEMORY.md</code> too. Document knowledge lives beside it in dedicated tables.</span></div>`;
    h += `<div class="card">
      <div class="u" style="font-family:var(--mono);font-size:12.5px;word-break:break-all">${esc(db.path)}</div>
      <div class="meta">${kb} KB · PostgreSQL + pgvector · schema is isolated per OTTO_HOME</div>
      <div class="meta" style="margin-top:8px"><code>psql -d ${esc(db.database||"otto")}</code> &nbsp;·&nbsp; ${reveal("","open the .otto folder")}</div></div>`;
    ...
    h += `<h2>Search indexes</h2><div class="card">GIN indexes provide PostgreSQL full-text retrieval;
      <code>knowledge_chunks_embedding_idx</code> is an HNSW cosine index powered by pgvector.
  ...
  ops(d){
    ...
    h += `<div class="card"><span class="r">Every LLM call's tokens are logged to
      <code>.otto/usage.jsonl</code> (append-only, never wiped). Dollar cost is estimated from tokens
    ...
    h += (d.trace_tail||[]).length ? table(["event","detail","when"], d.trace_tail.map(e =>
      ...
      : `<div class="card empty">no trace lines yet — talk to Otto</div>`;
