// otto dashboard — render/refresh loop, resizers/chrome, voice, bootstrap (LOADS LAST).
...
function render(){
  ...
  document.getElementById("n-tasks").textContent = (D.tasks||[]).filter(t=>t.status!=="completed").length || "";
...
const micHint = (msg) => { const i = document.getElementById("dmsg");
  if (i){ i.placeholder = msg; setTimeout(()=>{ i.placeholder = "Message Otto…"; }, 8000); } };
