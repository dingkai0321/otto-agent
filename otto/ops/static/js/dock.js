// otto dashboard — chat sessions/history (loadThreadInto), model chip, stats toggle.
...
// thread), keep it live-synced (so new gateway messages appear), and make
...
function applyTele(){
  const off = localStorage.getItem("otto_tele") === "0";
...
function toggleTele(){
  const off = localStorage.getItem("otto_tele") === "0";
  localStorage.setItem("otto_tele", off ? "1" : "0");   // flip
  applyTele();
}
