// otto dashboard — Model arena: race ONE message through several models at once.
...
try {
  const saved = JSON.parse(localStorage.getItem("otto_compare") || "null");
...
function saveCompare(){
  try { localStorage.setItem("otto_compare", JSON.stringify({
...
// Write to the real Apple Calendar ('Otto' calendar), opt-in. OFF by default so
...
VIEWS.compare = function(d){
  ...
  return `<div class="card">
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:6px">
      ...
      <label class="cmp-judge ${compareState.apple?"on":""}" style="margin-left:auto" title="Write create_event results to your REAL Apple Calendar (the 'Otto' calendar). Off by default so a race doesn't spam duplicates — when on, EACH model writes its own event (use 1-2 models).">
