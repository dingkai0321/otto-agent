...
from otto.graph.engine import END, START, Graph, Node
from otto.graph.nodes import key_router
...
QUICK_REPLY_PROMPT = """\
You are Otto, a warm, concise personal assistant. The user's message needs no
...
def build_triage_graph(*, classify_fn: Callable[[str], tuple[str, str]],
    ...
    def classify(state: dict) -> dict:
        ...
        if hooks:  # the human-readable "why" — traced, and shown on the turn card
            from otto.hooks import HookEvent

            hooks.trigger(HookEvent.TRIAGE_DECISION, route=route, reason=reason)
