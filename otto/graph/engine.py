...
from otto.hooks import HookContext, HookEvent, HookManager, HookResult, Observer, build_hooks
...
def run_graph(graph: Graph, state: dict, observer: Observer | None = None,
              max_steps: int = 25, hooks: HookManager | None = None) -> dict:
    ...
    manager = hooks or build_hooks(None, observer)
    ...
    manager.trigger(HookEvent.GRAPH_START, workflow=graph.name, nodes=list(graph.nodes))
    ...
    def run_one(name: str) -> tuple[str, dict | None, str | None, int]:
        ...
        snapshot = dict(state)
        node_hooks = manager.fork()

        def tag_node(_ctx: HookContext, _name=name) -> HookResult:
            return HookResult(updates={"node": _name})

        node_hooks.register(
            "*", tag_node, name="graph_node_tag", priority=9_000, terminal=True
        )
        snapshot["_hooks"] = node_hooks
        t = time.perf_counter()
        try:
    ...
    while wave:
        ...
        for name in wave:
            runs[name] += 1
            manager.trigger(
                HookEvent.GRAPH_NODE_START,
                workflow=graph.name, node=name, visit=runs[name],
            )
        ...
        for name, out, error, ms in results:  # merge in wave order — deterministic
            ...
            for key in keys:
                ...
                state[key] = out[key]
            manager.trigger(
                HookEvent.GRAPH_NODE_END,
                workflow=graph.name, node=name, ms=ms, keys=keys, error=error,
            )
            if error:
                errors[name] = error
            ...
            if name in graph.routers:
                ...
                manager.trigger(
                    HookEvent.GRAPH_ROUTE,
                    workflow=graph.name, router=name, target=target or END, reason=label,
                )
                if target is None:
