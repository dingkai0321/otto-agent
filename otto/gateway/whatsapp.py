...
Setup (10-20 minutes, free tier available but painful):
  ...
  5. Set up webhook verification (WHATSAPP_VERIFY_TOKEN):
     - Pick any random string, e.g. "my-otto-verify-token-123"
...
from otto.gateway.cli import _observer  # mirror gate/tool activity on the laptop terminal
...
def _build_handler(
    ...
    s = load_settings()
    s.ensure_home()
    otto = Otto(settings=s, conn=connect(s.home, check_same_thread=False))
    otto.session.session_id = "whatsapp"
    otto_lock = threading.Lock()
...
def main() -> None:
    try:
        ...
        raise SystemExit("WhatsApp extra not installed: pip install 'otto-agent[whatsapp]'")
    ...
    if not token:
        raise SystemExit(
            ...
            "See otto/gateway/whatsapp.py docstring for setup instructions."
        )
...
def start_in_background() -> bool:
    ...
    `otto dashboard` runs the browser cockpit AND WhatsApp from one command.
    ...
    try:
        ...
        print("(whatsapp) WHATSAPP_TOKEN is set but the extra isn't installed — "
              "pip install 'otto-agent[whatsapp]'")
        return False
