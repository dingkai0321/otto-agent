...
only moves words in and out (that's the whole point of the gateway box).
  ...
  voice  macOS `say` with a British voice by default (zero setup), or the
         ...
         then set OTTO_TTS=kokoro  (OTTO_VOICE=bm_george / bm_fable / ...)
...
from otto.gateway.cli import _observer  # show gate/tool lines in voice mode too
...
class Ears:
    def __init__(self, model_size: str | None = None):
        ...
        self.model = WhisperModel(
            model_size or os.getenv("OTTO_WHISPER_MODEL", "base"),
            compute_type="int8",
        )

    def transcribe(self, audio, language: str | None = None) -> str:
        segments, _ = self.model.transcribe(
            audio, language=language or os.getenv("OTTO_WHISPER_LANG")
        )
...
class Mouth:
    ...
    def __init__(self):
        self.engine = os.getenv("OTTO_TTS", "").strip().lower()
        self.voice = os.getenv("OTTO_VOICE", "")
        if not self.engine:
    ...
    def speak(self, text: str) -> None:
        ...
        if self.engine == "kokoro":
            ...
            print("(no TTS engine on this platform — set OTTO_TTS=kokoro)")


def matches_wake(text: str, wake_word: str) -> bool:
    """Does a transcript contain the (customizable) wake word?

    Fuzzy on purpose: Whisper hears "otto otto" as "ottootto", "Otto, otto!",
    "auto otto" — or transcribes it as Japanese kana わくわく (the first live
    test!). So: `wake_word` accepts comma-separated variants across scripts
    ("otto otto,わくわく"), normalization keeps kana AND CJK, and matching is
...
def _mic_threshold() -> float:
    ...
    OTTO_MIC_THRESHOLD (lower if it never hears you, higher if it
    wakes on room noise)."""
    return float(os.getenv("OTTO_MIC_THRESHOLD", "0.005"))
...
def wake_loop(otto: Otto, mouth: Mouth, wake_word: str) -> None:
    ...
    - The mic buffer is drained after Otto speaks, so it doesn't wake on
    ...
    ack = os.getenv("OTTO_WAKE_ACK", "Yes?")
    followup = float(os.getenv("OTTO_FOLLOWUP_SECONDS", "8"))  # stay open, Siri-style
    block = SAMPLE_RATE // 10
    # Pin the scout's transcription language to match the wake word's script —
    # otherwise Whisper hears "otto otto" and helpfully writes わくわく, which
    # a latin wake word never matches. Commands after wake still auto-detect.
    wake_lang = os.getenv("OTTO_WAKE_LANG") or ("en" if wake_word.isascii() else None)
        ...
        while True:
            ...
            if not matches_wake(heard_scan, wake_word):
                ...
                if heard_scan:  # near-misses belong in the trace (wake tuning!)
                    otto.emit(HookEvent.WAKE_SCAN, heard=heard_scan, matched=False)
                continue
            ...
            # for a follow-up for `followup` seconds — no need to say "otto otto"
            # again (like Siri). A quiet stretch drops back to wake-word mode.
            while True:
                heard = ears.transcribe(record_command(stream))
                if heard:
                    print(f"you › {heard}")
                    result = otto.respond(heard, observer=_observer, source="voice")
                    print(f"otto › {result.reply}")
...
def main() -> None:
    ...
    otto = Otto()
    otto.session.session_id = "voice"   # its own conversation thread in the inbox
    mouth = Mouth()

    # Hands-free by default: always-listening for "otto otto". The default packs
    # in the ways the tiny scanner mis-hears it (ottootto / waka waka / kana), so
    # it triggers reliably. Set OTTO_WAKE_WORD="" for push-to-talk instead.
    wake_word = os.getenv(
        "OTTO_WAKE_WORD", "otto otto,ottootto,otto,waka waka,wako wako,walk walk,わくわく"
    ).strip()
    if wake_word:
        try:
            wake_loop(otto, mouth, wake_word)
        except KeyboardInterrupt:
            pass
        print("\nbye — your memory stays in PostgreSQL")
        return

    ears = Ears()
    print("Voice Otto ready. Press Enter to talk, Ctrl-C to quit.")
    while True:
        try:
            input("\npress Enter to talk… ")
    ...
    print("bye — your memory stays in PostgreSQL")
