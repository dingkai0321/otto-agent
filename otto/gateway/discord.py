"""Discord gateway — message Otto from a direct message or a server channel.

READ THIS BEFORE PUTTING THE BOT IN A SHARED SERVER.

This gateway runs YOUR agent: your PostgreSQL memory, your SOUL.md, your
...
Setup:
  ...
  4. Open it up only as far as you actually need (all optional):
     ...
     DISCORD_HOME              a SEPARATE .otto for the bot. Use this whenever
...
from otto.gateway.cli import _observer
...
def _strip_mention(content: str, bot_id: str) -> str:
    """Drop the '@Otto Agent' prefix so the model sees the question, not the ping."""
    for form in (f"<@{bot_id}>", f"<@!{bot_id}>"):
        content = content.replace(form, " ")
    return content.strip()


def _build_agent() -> Otto:
...
def _build_client():
    ...
    otto = _build_agent()
    otto.session.session_id = "discord"
    ...
    async def on_message(message: discord.Message) -> None:
        ...
        async with message.channel.typing():
            result = otto.respond(text, observer=_observer, source="discord")
...
def describe_posture() -> str:
    ...
    home = os.getenv("DISCORD_HOME", "").strip() or ".otto — YOUR personal memory"
...
def main() -> None:
    try:
        ...
        raise SystemExit("Discord extra not installed: pip install 'otto-agent[discord]'")

    token = os.getenv("DISCORD_BOT_TOKEN", "")
    if not token:
        raise SystemExit("Set DISCORD_BOT_TOKEN in .env (create a bot in the Discord Developer Portal).")
    client = _build_client()
    print("Otto is listening on Discord. Ctrl-C to stop.")
...
def start_in_background() -> bool:
    ...
    try:
        ...
        print("(discord) DISCORD_BOT_TOKEN is set but the extra isn't installed — "
              "pip install 'otto-agent[discord]'")
        return False
