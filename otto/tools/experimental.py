...
The division of labor is the teaching point: Otto is the orchestrator (memory,
working-memory assembly, evals, the human's context) and pi is the specialist
contractor (read/bash/edit/write, pure coding craft). Otto hires; pi codes;
Otto's release gate can then inspect the work.
...
The remaining two boxes are still SKELETONS on purpose. The former terminal
placeholder graduated into the core ``run_command`` tool with a real permission
surface. Everything here is OFF by default; set
`OTTO_EXPERIMENTAL=1` to register these tools.
"""
...
def _project_pi_flags() -> list[str]:
    ...
    This is the "pi x otto" payoff: otto upgrades its coding contractor without
    touching pi's source. pi auto-discovers project resources from cwd upward,
    but delegated runs happen in otto_workspace/ (outside the repo), so we pass
...
def _run_pi_json(cmd: list, workdir: Path, timeout: int, emit_subagent):
    """Run pi in --mode json, relaying curated events through the hook bus as they
    ...
    while True:
        ...
        if kind == "message_update":
            ...
            if delta.get("type") == "text_delta" and delta.get("delta"):
                emit_subagent({"agent": "pi", "type": "text", "delta": delta["delta"]})
            ...
            for name in tools_called:
                emit_subagent({"agent": "pi", "type": "tool", "tool": name})
        elif kind == "turn_end":
            emit_subagent({"agent": "pi", "type": "turn_end",
...
def make_delegate_tool(settings: Settings) -> Tool:
    ...
    Same honesty contract as every Otto tool — the return string says exactly
    ...
    def delegate_task(task: str = "", cwd: str = "", timeout_seconds: int = 0,
                      _hooks=None, _notify=None) -> str:
        # _notify is a compatibility seam for direct tests/extensions. Normal
        # agent execution supplies the one run-scoped HookManager.
        if _hooks is not None:
            from otto.hooks import HookEvent

            emit_subagent = lambda ev: _hooks.trigger(HookEvent.SUBAGENT_EVENT, **ev)
        else:
            emit_subagent = (
                (lambda ev: _notify("subagent", ev)) if _notify else (lambda ev: None)
            )
        if not task.strip():
        ...
        from otto.tools import workspace
        if cwd:
        ...
        timeout = int(timeout_seconds) or int(os.getenv("OTTO_DELEGATE_TIMEOUT", "300"))
        ...
        from otto.ops.coding_eval import PI_PROVIDER, _key_for
        cmd = [pi_bin]
        ...
        if json_mode:
            try:
                code, reply, stderr, raw_events, tin, tout, cost = _run_pi_json(
                    cmd, workdir, timeout, emit_subagent)
            ...
            if code is None:
                return (f"pi was still working after {timeout}s so I stopped it — try a smaller "
                        f"task, or raise OTTO_DELEGATE_TIMEOUT.")
            stdout_text = reply
        else:
            try:
    ...
    return Tool(
        ...
        fn=delegate_task,
        wants_hooks=True,   # streams pi events through the shared lifecycle bus
    )
...
def make_tools(settings: Settings) -> list[Tool]:
    """Experimental tools, registered only when OTTO_EXPERIMENTAL=1: the live
