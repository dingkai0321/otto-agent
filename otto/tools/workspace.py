...
dir — so each scratch delegation instead lands in
    ...
    <OTTO_WORKSPACE>/<YYYY-MM-DD>/<HHMMSS>-<model>-<slug>/
        <the files pi wrote>
...
lives OUTSIDE .otto and is git-ignored — it never pollutes the agent's real
state or the repo.
...
WORKSPACE_ENV = "OTTO_WORKSPACE"          # root dir; default ./otto_workspace
AUTORUN_ENV = "OTTO_DELEGATE_AUTORUN"     # "0"/"false"/"no" to disable auto-run
RUN_TIMEOUT = int(os.getenv("OTTO_AUTORUN_TIMEOUT", "30"))
_OURS = {"MANIFEST.md", "run.log", "pi-transcript.log"}
_ENTRY_PREFS = ("main.py", "app.py", "run.py", "game.py")


def workspace_root() -> Path:
    return Path(os.getenv(WORKSPACE_ENV, "otto_workspace")).expanduser().resolve()
