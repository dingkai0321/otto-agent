...
Two OAuth-client sources, with the same read-only access:
  ...
  DEFAULT   otto's own OAuth client, shipped below. Click, approve in the
            browser, done. No files to download.

  OVERRIDE  your own `.otto/credentials.json`, when present. This replaces the
...
# otto's own OAuth client (Desktop app).
...
_SETUP_HINT = (
    "Google Calendar is not connected yet. Run `otto connect google` (or click "
...
def connect(home: Path) -> str:
    ...
    Uses otto's bundled client by default. If you dropped your own
    `.otto/credentials.json` in place, that OAuth client configuration wins.
        ...
        if own.exists():
            ...
            if not bundled.get("client_id") or not bundled.get("client_secret"):
                return (
                    ...
                    "yet. Add .otto/credentials.json to use your own Desktop "
                    "OAuth client."
                )
