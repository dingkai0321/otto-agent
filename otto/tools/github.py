...
Otto stores no GitHub token. It shells out to `gh`, which keeps its own
credentials in the system keychain — so nothing sensitive lands in `.env`, in
`.otto/`, or in a config file somebody might commit. If you have never run
...
_TIMEOUT = 20
# A PR diff is unbounded — otto-agent has had 400-line ones and the wider world
...
_REFUSAL = (
    "Otto's GitHub tool is read-only. Allowed commands: "
