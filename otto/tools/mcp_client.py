"""MCP connector — plug any Model Context Protocol server into Otto's tools.

Otto's loop is synchronous; the MCP SDK is async. The bridge below runs one
        ...
        for srv, metas in listed.items():
            for meta in metas:
                tools.append(Tool(
                    ...
                    permission_reason=meta.get("permission_reason", "MCP tool has unknown side effects"),
                ))
        return tools
            ...
            try:
                ...
                for tool in tools:
                    ...
                    if destructive is True:
                        permission, reason = "ask", "MCP tool declares destructive side effects"
                    elif read_only is True:
                        permission, reason = "passthrough", "MCP tool declares read-only access"
                    else:
                        permission, reason = "ask", "MCP tool has unknown side effects"
                    rows.append({
                        "name": tool.name,
