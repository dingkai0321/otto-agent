"""Apple-ecosystem tools (macOS) so Otto can brief you on your real week —
reading your actual Calendar.app (including email-invited events) and Mail, and
writing Reminders/Notes. Opt-in via OTTO_APPLE_TOOLS=1; first use triggers the
...
def read_apple_calendar(days_ahead: int = 7) -> str:
    ...
    Set OTTO_APPLE_CALENDARS=Work,Home to name the calendars you actually care
    ...
    """
    def go() -> str:
        cals = [c.strip() for c in os.getenv("OTTO_APPLE_CALENDARS", "").split(",") if c.strip()]
        if not cals:
            return (
                "Calendar not configured. Set OTTO_APPLE_CALENDARS to the calendars you "
                "actually use (e.g. OTTO_APPLE_CALENDARS=Work,Home) — a typical Mac has "
...
def make_tools() -> list[Tool]:
    return [
        ...
        Tool("create_reminder",
             ...
             permission="ask", permission_reason="writing to Apple Reminders"),
        Tool("create_note",
             ...
             permission="ask", permission_reason="writing to Apple Notes"),
    ]
