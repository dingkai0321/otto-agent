...
Where events land:
  always      PostgreSQL (the eval asserts here) + calendar.ics (importable file)
  opt-in      Apple Calendar, in a dedicated "Otto" calendar, via AppleScript —
              set OTTO_APPLE_CALENDAR=1. First use makes macOS ask permission
              for your terminal to control Calendar; approve once.
  opt-in      Google Calendar via OTTO_GOOGLE_CALENDAR=1. Local files remain
...
relays it, so Otto never over-claims what happened.
"""
...
def sync_to_apple_calendar(title: str, start: str, end: str, notes: str = "") -> str:
    """Create the event in Calendar.app under the 'Otto' calendar (created on
    ...
    # Prefer a dedicated "Otto" calendar, but macOS can't create calendars in
...
def make_tool(
    ...
    def create_event(
        ...
        existing = conn.execute(
            "SELECT id FROM calendar_events WHERE title = %s AND start = %s", (title, start)
        ).fetchone()
        if existing:
            return f"Event '{title}' at {start} already exists (not duplicated)."

        conn.execute(
            'INSERT INTO calendar_events (title, start, "end", attendees, notes) VALUES (%s,%s,%s,%s,%s)',
        ...
        if not apple_calendar and not google_calendar:
            where += (
                " Not synced to any calendar app (enable with OTTO_APPLE_CALENDAR=1 "
                "or OTTO_GOOGLE_CALENDAR=1, "
    ...
    return Tool(
        ...
        fn=create_event,
        permission="ask" if (apple_calendar or google_calendar) else "passthrough",
        permission_reason=(
            "creating an event in an external calendar"
            if (apple_calendar or google_calendar) else ""
        ),
    )


def make_list_tool(conn, home: Path | None = None) -> Tool:
    ...
    schedule; the local PostgreSQL calendar second, because it only ever holds what
    otto itself created. Every source is LABELLED in the output, so the agent can
    ...
    def list_events(start: str = "", end: str = "", limit: int = 20) -> str:
        ...
        if home is not None:
            from otto.tools.google_calendar import is_connected, list_google_events
        ...
        checked.append("otto's local calendar")
        local = local_events(start, end, limit)
        if local:
            sections.append("From otto's local calendar (events otto created):\n" + local)
    ...
    return Tool(
        ...
        description=(
            ...
            "when signed in, plus otto's own local calendar). "
