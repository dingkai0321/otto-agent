...
description: >
  ...
  for otto-agent, without leaking API keys or leaving branches behind. Use when
...
git -C $REPO worktree add -q ~/Developer/otto-prs/pr$N pr-$N
ln -sfn $REPO/.otto ~/Developer/otto-prs/pr$N/.otto   # real memory/traces/calendar
```
...
```bash
ln -sfn $REPO/.env ~/Developer/otto-prs/pr$N/.env
```
...
```bash
[ -L ~/Developer/otto-prs/pr$N/.env ] && echo "symlink (safe)" || echo "REAL FILE — copy of all keys"
```
...
what the live demo shows (e.g. `OTTO_EPISODIC_STORE`) on their old value — Sean
...
```bash
cd ~/Developer/otto-prs/pr$N
OTTO_DASHBOARD_PORT=7778 $REPO/.venv/bin/python -m otto.ops.dashboard
```
...
for d in ~/Developer/otto-prs/*/; do git -C $REPO worktree remove --force "$d"; done
git -C $REPO worktree prune
git -C $REPO branch --list 'pr-*' | tr -d ' ' | xargs -r -n1 git -C $REPO branch -D
# 4. verify nothing is left holding keys
find ~/Developer/otto-prs -name ".env" 2>/dev/null
```
...
- [ ] `find ~/Developer/otto-prs -name ".env"` prints nothing
