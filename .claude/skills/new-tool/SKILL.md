---
name: new-tool
description: Add a new tool to Otto the right way — schema, safe execution, deterministic eval, honest output. Use when adding or modifying agent tools or when the user asks for a new capability.
---

## Procedure

1. One file per tool in `otto/tools/`, exposing `make_tool(...) -> Tool`.
   Copy the shape of `otto/tools/calendar.py` (the reference implementation).
2. The tool function must be:
   ...
   - **Honest in its return string**: say exactly where the artifact went
     ...
     tool doesn't have.
   - **Local-first by default**: write to `.otto/` or state.db; real external
     services go behind env-gated adapters.
3. Register it in `otto/tools/__init__.py:build_registry`.
...
5. If the tool needs usage guidance, add a rule to DEFAULT_SOUL
   (`otto/runtime/session.py`) or a skill in `skills/`.
