# otto-agent — one command per pillar.
#
...
.PHONY: run voice discord whatsapp brief dashboard trace eval eval-judge gate lint

run:            ## chat with Otto in the terminal
	$(PY) -m otto

voice:          ## talk to it — push-to-talk, or always-on with OTTO_WAKE_WORD
	$(PY) -m otto voice
...
lint:
	$(PY) -m ruff check otto evals scripts
