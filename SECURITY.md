...
otto runs on your own machine, with your own API keys, and reads your own
...
[private vulnerability reporting](https://github.com/ShenSeanChen/otto-agent/security/advisories/new),
...
- Anything that exfiltrates keys, `.env`, PostgreSQL memory, traces, or
...
- **`OTTO_EXPERIMENTAL=1`** enables sub-agent delegation, which runs another
  coding agent locally. It's off by default and documented as experimental.
- **Your own API keys in your own `.env`.** otto never sends them anywhere but
...
- Inbound gateways (WhatsApp-style webhooks) must verify request signatures
  before acting. Outbound ones (such as Discord) dial out and aren't exposed.
